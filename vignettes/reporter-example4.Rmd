---
title: "Full Program 4: Figure"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Full Program 4: Figure}
...
can be used to create figures.  The figures are created via the popular
**ggplot2** package, and then appended as content to the report.  There
is no need to export the image to a file.  Simply pass the entire **ggplot2**
object to the `create_plot()` function, and the system will take care of 
the rest.

This example also shows how multiple 
pieces of content can be added to a single report using the 
`add_content()` function. Content
can be appended to the report endlessly. You can append tables, 
figures, and textual analysis onto the same report.  The `page_break`
parameter on the `add_content()` function allows you to control whether
or not page breaks should occur in between pieces of content.
...
<img src="../man/images/example4a.png"/>
<img src="../man/images/example4b.png"/>
<img src="../man/images/example4c.png"/>
...
Program Path: C:\packages\Testing\reporter_example4.R
...
Next: [Full Program 5: Survival Analysis](reporter-example5.html)
