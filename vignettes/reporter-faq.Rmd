...
paging variable.
However, if your paging variable are detected to cause an overflow, the 
**reporter** package package will still wrap to the next page on its own. You
can use `report_options(auto_page = FALSE)` to let pagination 100% follow your
paging variable, but you need to be responsible for the overflow.
See [Example 18: Report Options](reporter-report_options.html)

You can also set the number of lines on a page using the `line_count`
parameter on the `options_fixed()` and `report_options` function. When this 
parameter is set, it will use the number of lines specified to 
determine when to break a page, instead of using the automatically calculated value. 
...
In default, , the **reporter** would wrap columns across pages when there are 
too many columns to fit in one page without any manual setting. You can turn it 
off with `report_options(page_wrap = FALSE)` or `create_table(page_wrap = FALSE)`.
See [Example 18: Report Options](reporter-report_options.html)
...
In other words, use "{{mystring}}" instead of "{mystring}". If you're using RTF 
code with `report_options(allow_code = TRUE)`, please also double the curly braces
if needed. See [Example 17: Insert RTF Code](reporter-RTFcode.html)
