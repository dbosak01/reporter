...
#' @import magrittr
#' @export
write_report_docx <- function(x, graphic_type="png"){
  ...
  for(o in ls){
    ...
    if (class(o)[1] == "table_spec"){
      ...
      fts <- create_flextables(o, body_size = bs, font_name = x$font_name)  
  ...
  xml <- gsub("[\r\n]\\s+", "",
     ...
     <w:body>
        ...
        <w:sectPr w:rsidR="001F5D69" w:rsidRPr="0072030C" w:rsidSect="00987FC6">
           ...
           <w:pgMar w:top="{margin_top}" w:right="{margin_right}" 
           w:bottom="{margin_bottom}" w:left="{margin_left}" 
...
#' Writes the page template to the file system.  The page template is used
